...
askInstall()
{
  ...
  if [[ "$answer" == [Yy] ]]; then
    ...
    echo -n "Installing $1: "
    chmod a+x $1
    sudo cp $1 /usr/bin > /dev/null 2>&1 || { echo "Failure"; echo "Error copying file, try running install script as sudo"; exit 1; }
    echo "Success"
    cd .. || return 1
  fi
}
...
singleInstall()
{
  ...
  echo -n "Installing $1: "
  chmod a+x $1
  sudo cp $1 $prefix/bin > /dev/null 2>&1 || { echo "Failure"; echo "Error copying file, try running install script as sudo"; exit 1; }
  echo "Success"
  cd .. || exit 1
}
